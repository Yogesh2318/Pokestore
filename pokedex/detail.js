    ...
    fetch(`https://pokeapi.co/api/v2/pokemon-species/${id}`)
       ...
       if(currentPokemonId===id){
        displayPokemonDetails(pokemon)
            const flavorText = getEnglishFalvorText
            (pokemonspecies);
            ...
            window.history.pushState({},"",`./poke.html?id=${id}`);
...
function setTypeBackgroundColor(pokemon){
  ...
  steel: "#B8B8D0",
    ...
    if(!color){
        console.warn(`Color not define foer type: ${mainType}`);
        return;
    }
    const detailMainElement = document.querySelector(".detail-main");
    detailMainElement.style.background = color;
    setElementStyles([detailMainElement],
        ...
        setElementStyles([detailMainElement],
            ...
            setElementStyles(
                document.querySelectorAll(".stats-wrapper > p"),
                "BackgroundColor",
                color
            );
            setElementStyles(
